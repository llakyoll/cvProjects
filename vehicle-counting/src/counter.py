"""Two-line corridor counter for tracked vehicles.

Counts a track only once it has crossed both lines of a corridor in
sequence, not just touched a single line. This filters out vehicles that
...
class CorridorCounter:
    ...
    is counted "in" once it crosses near -> far (increasing coordinate),
    and "out" once it crosses far -> near (decreasing coordinate). Crossing
    only one of the two lines does not count.
